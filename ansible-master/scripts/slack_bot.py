...
def does(args_str, say):
    # Import here to avoid circular imports
    from does_master import does_master_exec
    return does_master_exec(args_str, say)
...
def bot_handle(cmd):
    ...
    if cmd.cmd not in KNOWN_CMDS:
        ...
        text, mrkdwn_strs, files_to_upload = KNOWN_CMDS[cmd.cmd](cmd.args, cmd.say)
    ...
    if files_to_upload:
        ...
        cmd.say(blocks=mrkdown_blocks, text=text)
    else:
        cmd.say(text)
...
if __name__ == "__main__":
    ...
    # Import here to avoid circular imports
    from does_master import does_fetch_results, DEFAULT_PLOTS_SUBDIR
    ...
    parser.add_argument("-p", "--plots", nargs="+", default=[DEFAULT_PLOTS_SUBDIR], type=str,
        help="Only fetch plots from the specified subfolder in does_results ")

    args = parser.parse_args()
    commit = args.commit
    channel = args.channel
    plots_subdirs = args.plots

    if not commit:
        handler = SocketModeHandler(app, SLACK_APP_TOKEN)
        handler.start()
    elif channel:
        texts, _, files_to_upload = does_fetch_results([commit], plots_subdirs)
        app.client.files_upload(
            channels=channel,
