...
class DemoLatencyPlotLoader(PlotLoader):
    def load(self, df: pd.DataFrame, options: Dict, etl_info: Dict) -> None:
        if not df.empty:
            ...
            self.save_plot(fig, filename=f"test", output_dir=output_dir)
...
class CoordinateLoader(PlotLoader):
    ...
    def load(self, df: pd.DataFrame, options: Dict, etl_info: Dict) -> None:
        if not df.empty:
            fig = self.plot(df)
            output_dir = self.get_output_dir(options, etl_info)
            self.save_plot(fig, filename=f"{etl_info['pipeline']}_coord", output_dir=output_dir, output_filetypes=['png'])
