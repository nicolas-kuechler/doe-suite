...
class FilterColumnTransformer(Transformer):
    ...
    def transform(self, df: pd.DataFrame, options: Dict) -> pd.DataFrame:
        ...
        import warnings
        warnings.warn("FilterColumnTransformer is deprecated, instead you can directly use df.query(col == 'A') in the etl definition i.e., transformers: [df.query: {expr: col == 'A'}]", DeprecationWarning)
